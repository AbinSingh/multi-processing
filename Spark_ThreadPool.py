...
def process_file(file_path):
    ...
    if extracted_text is None:
        logger.warning(f"Skipping file due to extraction failure: {file_path}")
        return None

    api_response = call_gpt_model(extracted_text)
    if api_response is None:
        logger.warning(f"Skipping file due to API failure: {file_path}")
        return None
    ...
    logger.info(f"Processed file {file_path} with response {structured_response}")
